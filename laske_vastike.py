...
class vastikelaskelma():
  ...
  def __init__(self):
    with open('data.txt') as data_file:    
      ...
      mailObj = hae_postit()
      self.timeframe, self.kausiStr = mailObj.get_current_timeframe()
  ...
  def yhtion_lampiman_veden_kulutus(self, vuosi, kuukausi):
    taloyhtion_mittarilukema_nyt = self.vastike["yhtionmittarit"][self.kausi(vuosi, kuukausi)]["vesimittari"]
    taloyhtion_mittarilukema_viimekuussa = self.vastike["yhtionmittarit"][self.kausi(vuosi, kuukausi-1)]["vesimittari"]
    ...
    for talo in "ABCDEFG":
      kylma_vesi_nyt = self.vastike["talot"][talo][self.kausi(vuosi, kuukausi)]["KylmaVesi"]
      kylma_vesi_viimekuussa = self.vastike["talot"][talo][self.kausi(vuosi, kuukausi-1)]["KylmaVesi"]
  ...
  def lammityksen_kulutus_per_talo(self, vuosi, kuukausi):
    ...
    totaali_kulutus = 0
    for talo in "ABCDEFG":
      lammitys_nyt = self.vastike["talot"][talo][self.kausi(vuosi, kuukausi)]["Lammitys"]
      lammitys_viimekuussa = self.vastike["talot"][talo][self.kausi(vuosi, kuukausi-1)]["Lammitys"]
  ...
  def kierto_per_talo(self, vuosi, kuukausi):
    ...
    kiertovesiTotal = 0
    for talo in "ABCDEFG":
      kiertovesi_nyt = self.vastike["talot"][talo][self.kausi(vuosi, kuukausi)]["KiertoVesi"]
      kiertovesi_viimekuussa = self.vastike["talot"][talo][self.kausi(vuosi, kuukausi-1)]["KiertoVesi"]
  ...
  def kausi(self, vuosi, kuukausi):
    return self.kausiStr
  ...
  def kaukolammon_kokonaiskulutus(self, vuosi, kuukausi):
    taloyhtion_mittarilukema_nyt = self.vastike["yhtionmittarit"][self.kausi(vuosi, kuukausi)]["kaukolampo"]
    taloyhtion_mittarilukema_viimekuussa = self.vastike["yhtionmittarit"][self.kausi(vuosi, kuukausi-1)]["kaukolampo"]
    kaukolampo_kokonaiskulutus = (taloyhtion_mittarilukema_nyt - taloyhtion_mittarilukema_viimekuussa)/1000
    return kaukolampo_kokonaiskulutus
    

  def hae_muut_yhtion_menot(self, vuosi, kuukausi):
    muut_kulut = self.vastike["yhtionmenot"][self.kausi(vuosi, kuukausi)]
    summa = 0
